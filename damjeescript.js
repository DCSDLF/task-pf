...
document.getElementById('uploadBtn').addEventListener('click', function() {
    ...
    if (files.some(file => !file)) {
        alert('Please upload both meter data CSV files.');
        return;
    }
...
document.getElementById('downloadBtn').addEventListener('click', function() {
    ...
    const numberFormatColumns = [1, 2, 3, 4, 5]; // Columns 2, 5, 4, 3, 6 (0-indexed)
...
function displayData(dataArrays, output) {
    // Prepare to store the filtered data for each meter
    const maxRows = Math.max(dataArrays[0].length, dataArrays[1].length);
    const filteredData = [];
        ...
        for (let j = 0; j < dataArrays.length; j++) {
            if (dataArrays[j][i]) {
                if (j === 0) {
                    ...
                    row.push(dataArrays[j][i][12] || ''); // Column 32
                    row.push(dataArrays[j][i][13] || ''); // Column 34
                } else {
                    // For Meter 2, only keep the relevant columns
                    row.push('', dataArrays[j][i][12] || '', dataArrays[j][i][13] || ''); // Empty for Date, keep columns 32 and 34
                }
            } else {
    ...
    output.innerHTML += '<strong>Meter 1 (Date) | Meter 2</strong><br>';
