...
document.getElementById('uploadBtn').addEventListener('click', function() {
    ...
    if (files.some(file => !file)) {
        alert('Please upload all three meter data CSV files.');
        return;
    }
...
function displayData(dataArrays, output) {
    // Prepare to store the filtered data for each meter
    const maxRows = Math.max(dataArrays[0].length, dataArrays[1].length, dataArrays[2].length);
    const filteredData = [];
        ...
        for (let j = 0; j < dataArrays.length; j++) {
            if (dataArrays[j][i]) {
                if (j === 0) {
                    ...
                    row.push(dataArrays[j][i][21] || ''); // Column 34
                } else {
                    // For Meter 2 and Meter 3, only keep the relevant columns
                    row.push('', dataArrays[j][i][20] || '', dataArrays[j][i][21] || ''); // Empty for Date, keep columns 32 and 34
                }
            } else {
    ...
    output.innerHTML += '<strong>Meter 1 (Date) | Meter 2 | Meter 3</strong><br>';
...
document.getElementById('downloadBtn').addEventListener('click', function() {
    ...
    const numberFormatColumns = [1, 2, 3, 4, 5, 6, 7]; // Columns 2, 5, 8, 3, 6, 7, 9 (0-indexed)
